		...
		<!-- Two -->
			<section id="two" class="wrapper style2 special">
				<header class="major">
					...
					// Conta quantos registros existem na tabela para o usuário		
					$vConsultaCount = "SELECT count(t1.id_pergunta) cnt FROM perguntas t1 where t1.id_usuario = {$vIdUsuario} and t1.id_status_mensagem = 2";
					$vRetornoCount = $mysqli->query($vConsultaCount) or die("ERR03 - ERRO NO BANCO DE DADOS!");
					...
					if ($vCount > 0) {
						...
						// Consulta última pergunta realizada e verifica se há uma resposta.
						$vConsulta = "SELECT t1.id_pergunta, t2.desc_prioridade, t1.msg_pergunta, t1.dt_ins FROM perguntas t1 inner join prioridade t2
						  on t1.id_prioridade = t2.id_prioridade where t1.id_usuario = {$vIdUsuario} and t1.id_status_mensagem = 2 and
						  t1.id_pergunta = (select max(id_pergunta) from perguntas where id_usuario = {$vIdUsuario} and id_status_mensagem = 2)";

						$vRetorno = $mysqli->query($vConsulta) or die("ERR04 - Erro no banco de dados!"); 
						$vLinha = $vRetorno->fetch_assoc();
				...
				<ul class="actions">
					<li><a href="enviar.php" class="button">Enviar Nova Mensagem</a></li>
					<?php
						if ($vCount > 0) {
							echo "<li><a href=\"inbox.php\" class=\"button\">InBox</a></li>";
						}
					?>
					<li><a href="#" class="button">Voltar</a></li>
				</ul>
