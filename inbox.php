...
<html>
	...
	<?php
		...
		$vConsultaCount = "SELECT count(t1.id_pergunta) cnt FROM perguntas t1 where t1.id_usuario = {$vIdUsuario}";
			...
			<section id="one" class="wrapper style2 special">
				...
				<h3>InBox</h3>
					...
					while ($vLinhaPergunta = $vRetornoPergunta->fetch_assoc())
					{
						...
						if (strlen($vMsgPergunta) > 50) {
							echo "			<tr><td><strong>Pergunta</strong><br><a href=\"mensagem.php?id={$rIdPergunta}\">{$CSS_AbrirFonte}{$vMsgPergunta}{$CSS_FecharFonte}</a></td></tr>";
						} else {
							echo "			<tr><td><strong>Pergunta</strong><br>{$CSS_AbrirFonte}{$vMsgPergunta}{$CSS_FecharFonte}</td></tr>";
						}
						
						if (strlen($vMsgResposta) > 50) {
							echo "				<tr><td><strong>Resposta</strong><br><a href=\"mensagem.php?id={$rIdPergunta}\">{$CSS_AbrirFonte}{$vMsgResposta}{$CSS_FecharFonte}</a></td></tr>";
						} else {
